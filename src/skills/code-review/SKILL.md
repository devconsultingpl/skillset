...
Read-only review of *the changes*: is this delta correct, clear, and consistent with the codebase? Activates on "review my changes / this PR / this diff", or `/code-review`. Reviews the change — not the whole codebase. Pre-existing bloat is `declutter`'s, plan-alignment is `intent-review`'s, security depth is `security-review`'s.
...
- **Obvious security/perf** — visible injection, unsafe input, leaked secret, O(n²) in a hot path. Flag it; defer *deep* analysis to `security-review` or a perf pass. Never stay silent because "that's another skill's job."
