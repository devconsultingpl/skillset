...
Deep security lens on *the changes*: is this delta exploitable? Activates on "security review this / check for vulnerabilities", or `/security-review`. This is the dedicated deep pass — `code-review` flags *obvious* security issues in passing and defers depth here. Report-only: a wrong security fix is worse than none.
...
- An argument redirects to a path for a standalone audit regardless of diff: `/security-review src/auth`.
